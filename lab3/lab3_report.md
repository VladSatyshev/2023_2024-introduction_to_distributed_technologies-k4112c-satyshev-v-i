...
Для того, чтобы браузер дал доступ к приложению требуется создать собственный Certificate Authority, с помощью которого сгенерировать TLS сертификат.
Набор действий, который необходимо выполнить для достижения поставленной цели основывается на [статье](https://devopscube.com/create-self-signed-certificates-openssl/) и представлен на рисунке:
