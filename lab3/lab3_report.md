...
Отметим, что в отличии от работы №1 и №2 в данном случае допускается использовать тип сервиса `ClusterIP`, а не `NodePort`, т.к. доступ извне будет осуществляться посредством `Ingress`, который создается в п. 4 данной работы. Заметим также, что установлен 443 `port` по которому доступен `Service` внутри кластера, т.к. будет использоваться https соединение.
...
Набор действий, который необходимо выполнить для достижения указанной цели, основывается на [статье](https://devopscube.com/create-self-signed-certificates-openssl/) и представлен на рисунке:
...
- `subjectAltName` - расширение альтернативного имени субъекта позволяет включать в файл конфигурации различные литеральные значения. К ним относятся адрес электронной почты, URI (Uniform Resource indicator), DNS имя, RID, IP-адрес, dirName (distinguished name) и otherName.
